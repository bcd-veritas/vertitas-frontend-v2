...
const FADE_IN = 1;
/** Seconds for the boost to wind up, as an exponential time constant: the ramp
 *  covers ~63% of the gap per tau, ~90% by 2.3 tau. Snapping straight to the
 *  boost speed read as a hard cut, so the launch eases in from cruising speed
 *  instead. Kept under the 700ms launch delay so it is near full speed by the
 *  time the DOM zoom takes over. */
const BOOST_RAMP_TAU = 0.3;
...
export default function ImageBox(props: Partial<ImageBoxProps>) {
  ...
  const onLaunchRef = useRef<(() => void) | undefined>(undefined);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    let raf = 0;
    let last = 0;
    /** Current per-frame scroll step, eased toward the target. -1 = unset, so
     *  the first frame adopts the cruising speed instead of ramping from zero
     *  and visibly stuttering on mount. */
    let currentSpeed = -1;
    let boosted = false;
    let launching = false;
    ...
    const animate = (now: number) => {
      ...
      // Ease toward the target rather than switching to it. Exponential
      // approach keyed off dt, so the ramp takes the same wall-clock time
      // regardless of frame rate.
      const targetSpeed = boosted ? cfg.boost : cfg.speed;
      if (currentSpeed < 0) currentSpeed = targetSpeed;
      currentSpeed +=
        (targetSpeed - currentSpeed) * (1 - Math.exp(-dt / BOOST_RAMP_TAU));
    ...
    const onMove = (e: PointerEvent) => {
      // Once launched the label is gone for good — stop tracking so it can't be
      // dragged back into view mid-transition.
      if (launching) return;
    ...
    const onDown = () => {
      ...
      launching = true;
      boosted = true;
      // The label has done its job — retire it on the press rather than leaving
      // "Explore" floating over the flythrough.
      const el = cursorRef.current;
      if (el) {
        el.style.opacity = "0";
        el.style.transform = "translate(0%, -100%) scale(0.85)";
      }
    ...
    return () => {
      ...
      if (launchTimeout !== undefined) clearTimeout(launchTimeout);
      ro.disconnect();
  ...
  }, [urls, palette, background, lineColor, lineOpacity, grid, fade, launchDelayMs]);
