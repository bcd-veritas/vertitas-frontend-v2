        ...
        <tbody>
          {trades.map((t) => (
            <tr key={t.id} className="border-t border-line/60">
              <td className="py-1.5 pr-2 font-mono text-[11px] text-muted">{timeAgo(t.createdAt)}</td>
              <td className="px-2 font-mono text-[10px] uppercase tracking-wider text-fg/80">
                {outcomeLabels[t.outcomeIndex] ?? `#${t.outcomeIndex}`}
              </td>
              <td className="px-2 text-right font-mono text-xs tabular-nums text-fg/80">{centsLabel(t.price)}</td>
              <td className="px-2 text-right font-mono text-xs tabular-nums text-muted">{sharesLabel(t.quantity)}</td>
