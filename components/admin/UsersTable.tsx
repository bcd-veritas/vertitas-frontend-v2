...
function isRowEditable(
  ...
  if (row.walletAddress.toLowerCase() === actorAddress.toLowerCase())
    return false; // self
    ...
    <>
      {Array.from({ length: 8 }).map((_, i) => (
        <tr key={i} className="border-t border-line/60">
          ...
          <td className="px-2">
            <div className="h-4 w-16 animate-pulse bg-fg/5" />
          </td>
          <td className="px-2">
            <div className="h-3 w-14 animate-pulse bg-fg/5" />
          </td>
          <td className="px-2">
            <div className="ml-auto h-3 w-12 animate-pulse bg-fg/5" />
          </td>
          <td className="px-2">
            <div className="ml-auto h-3 w-12 animate-pulse bg-fg/5" />
          </td>
          <td className="pl-2">
            <div className="ml-auto h-3 w-14 animate-pulse bg-fg/5" />
          </td>
        </tr>
      ))}
    </>
...
export function UsersTable() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (_result, variables) => {
      ...
      if (
        isOracleRole(variables.newRole) ||
        isOracleRole(variables.user.role)
      ) {
                      ...
                      {isRowEditable(actorRole, address, u) ? (
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            if (next !== u.role)
                              setPending({ user: u, newRole: next });
                          }}
                        ...
                        >
                          {Array.from(
                            new Set([u.role, ...selectableRoles(actorRole)]),
                          ).map((r) => (
                ...
                <div className="mt-5 flex justify-end gap-2">
                  ...
                  <button
                    ...
                    className={`border border-fg bg-fg px-4 py-2 font-mono text-[11px] uppercase tracking-wider text-bg transition-opacity hover:opacity-85 disabled:opacity-40 ${
                      mutation.isPending
                        ? 'cta-busy relative overflow-hidden'
                        : ''
                    }`}
                  >
